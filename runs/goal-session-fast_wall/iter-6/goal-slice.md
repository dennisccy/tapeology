<!-- GOAL SLICE: generated by goal_gate.py. Stable passing journeys are
     digested to one line (6 of 7); vision, anti-goals, and
...
- **J-01: Stop the bleeding — GET /research/edge-report never computes** — passing (stable; digested)
- **J-02: The stores stop re-reading — verified-content caches + durable dataset index** — passing (stable; digested)
- **J-03: The arm memo — per-tick levels recompute becomes ~100 memo hits per session** — passing (stable; digested)
- **J-04: The operator-run compute — button, background job, CLI warmer** — passing (stable; digested)
- **J-05: The sweep becomes resumable and parallel — durable pair results + process pool** — passing (stable; digested)
