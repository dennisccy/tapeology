...
## iter-13 — 2026-06-09T00:00:00Z

**Verdict:** GOAL_ACHIEVED
**Lesson:** Re-tuning the classifier to relative (bps/return) gates was done safely by keeping the change PURELY ADDITIVE — relative path only fires when `reference_price > 0`, otherwise the exact pre-existing absolute constants run, so every legacy fixture stays byte-identical (test_classifier_relative pins the 0.8542 fallback confidence). The keystone that prevented a silent J-04/J-05 reclassification was making the absorption gate the EXACT complement of the control impact-return condition in the relative domain too (test_absorption_gate_is_exact_complement_of_control_impact_in_relative_domain), proven on the same reference shape at the boundary — not just spot-checked.
**Applies to:** any future iter touching `apps/backend/app/engine/classifier.py` or the spread/impact gate thresholds — prove the absorption/control complement on a shared fixture at the boundary, and assert the absolute-fallback fixtures are byte-identical.

## iter-14 — 2026-06-10T00:00:00Z

**Verdict:** GOAL_ACHIEVED
**Lesson:** A "spread <= cap" absolute veto in `classifier.py` silenced an obvious real >5% move because the single-venue IEX quoted spread was ~2,700 bps; the real fix was BOTH a per-mode feed (SIP for historical so the quoted spread is realistic, IEX kept for live) AND making spread a graded confidence factor within a config-owned band rather than a hard gate. The decisive proof that this is load-bearing (not a vacuous test) is a differential replay of the SAME committed real fixture with the override toggled off: seller_control @ 0.925 enabled vs unclear @ 0.200 disabled. Always do this toggle-off check when a real-data classification gate is asserted — a green test alone does not prove the new code path is what produced the pass.
**Applies to:** any iter touching `apps/backend/app/engine/classifier.py` spread/impact gates, the per-mode vendor feed in `alpaca.py`, or any real-data classification journey gated by a committed fixture (anti-goal #20).

## iter-14 — 2026-06-10T00:00:00Z

**Verdict:** GOAL_ACHIEVED
**Lesson:** "Chunked but still fetches the whole window before responding" (iter-13 J-34) is NOT the same as progressive loading — iter-13 parallelized within the 8s cap but `historical.py:stream()` still materialized the entire window, so time-to-first-data stayed coupled to total load. The real fix is a LAZY generator (`AlpacaAdapter.iter_historical_chunks`) consumed by a `ProgressiveHistoricalProvider` + background feeder; you cannot observe "before the whole window is fetched" with a pre-materialised fixture, so prove laziness with a counting fake SDK that records how many sub-windows were fetched after pulling only the first chunk. A dense ~17k-print real burst also forced an O(n^2) feature recompute — made incremental (O(1)-amortised, byte-identical via an eviction-fallback) so the engine warms in ~1s instead of stalling.
**Applies to:** any iter touching the historical fetch+replay seam (`alpaca.py` chunk iterator, `providers/historical.py`, `watch_manager.py` feeder) or claiming "fast by design" for a long-window vendor path; any engine hot-loop perf change (must prove byte-identical values through the eviction-fallback boundary).
