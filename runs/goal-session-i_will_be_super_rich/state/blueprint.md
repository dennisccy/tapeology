...
rows 10–12 are the **analysis-fidelity additions** (J-17/J-18, J-19, J-20 — to be built; J-16 is folded into row 4).
...
| 12 | **Resolved historical window** (tz-aware start/end instants for the user's selected **local** window; explicit zone label + US-session quick-picks) | **Frontend datetime module** (resolves the local selection → exact UTC instants **before** the fetch) → request body | `POST /watch/{ticker}` body `{mode:"historical",start,end,speed}` (timezone-aware instants) | Historical provider fetches exactly the resolved window (no silent UTC shift) |
