...
| PnL-ledger register / `reports/pnl/pnl-history.md` | `app/research/pnl_ledger.py` (+ `pnl_history.py` CLI) | `GET /research/pnl/ledger`, MCP `pnl_ledger` | Data Contract row 32 (era-3). Era-5B additively adds a `strategy_comparison` row kind (iter-9/J-08) composed **verbatim** from `edge_report.py`'s already-computed cells — `ledger_projection` / `render_history_markdown` stay the single read + renderer, no second path and no second computation. The 3-way table's band-level column is labelled **`band side`** (iter-10) to avoid collision with the pre-existing two-way row's role `side`; existing two-way rows render byte-identical. The first real 3-way append is operator-run over the credentialed corpus |
