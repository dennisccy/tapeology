...
@@ -257,7 +269,11 @@ function deskRowDrillInTitle(row: DeskScreenRow): string {
...
@@ -276,8 +292,8 @@ function deskSkipDrillInTitle(skip: DeskScreenSkip): string {
...
@@ -350,6 +366,18 @@ function DeskRow({ row, asOf }: { row: DeskScreenRow; asOf: string }) {
           ? "history not recorded in this snapshot"
...
+          row's ENTIRELY ABSENT key (`undefined`), not just an explicit `null`. */}
...
@@ -379,6 +407,7 @@ function DeskRowsTable({ rows, asOf }: { rows: DeskScreenRow[]; asOf: string })
