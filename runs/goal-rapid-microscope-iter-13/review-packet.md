...
- `apps/backend/tests/test_vault.py` (456 lines not shown)
...
@@ -1457,53 +1505,108 @@ def recover_shard_ledger(
     quarantine_dir: str,
...
+       HASH-ATTESTED COMPLETE -- FOUR conjuncts, every one required and none sufficient alone:
+       the candidate is non-empty, its row count equals the anchor's own ``row_count``, its final
+       row's hash equals the anchor's own ``head_hash``, and the whole candidate chain is
+       internally consistent when re-derived from its own row contents. The two guards beyond the
+       original count+hash pair each close a laundering path found by attacking this function
+       directly; the inline comments at the check itself carry both reproductions. See also the r8
+       note below on why the count conjunct can never be promoted into a dividing line of its own.
...
@@ -1527,32 +1630,36 @@ def recover_shard_ledger(
                 "outcome": "complete",
             }
         )
...
+    -- a strictly stronger guarantee that makes the old safety net unnecessary."""
     ...
     shard_ledger = vault.VaultShardLedger(vault_dir)
     universe_ledger = vault.VaultUniverseLedger(vault_dir)
@@ -1775,14 +1983,477 @@ def test_tc5_a_shard_recovery_could_not_name_stays_protected_by_the_universe_rul
...
... [diff_bound] apps/backend/tests/test_vault.py: 456 more diff lines omitted — Read the file for full detail
...
@@ -864,6 +899,7 @@ boundary by its `observed_through`.
...
## Excluded-path stat (dependency/lockfile visibility)
 ...
 .../state/blueprint.md                             | 27 ++++++++++-------
 runs/goal-session-rapid-microscope/telemetry.jsonl | 14 +++++++++
 .../trace/trace.jsonl                              |  4 +++
