...
- `apps/backend/app/research/scout.py` (543 lines not shown)
- `apps/backend/tests/test_scout.py` (257 lines not shown)
...
@@ -294,14 +352,6 @@ def get_walkforward_ledger_dir() -> str:
...
+    return anchors
+
... [diff_bound] apps/backend/app/research/scout.py: 543 more diff lines omitted — Read the file for full detail
...
+
 def test_extract_anchors_skips_a_dataset_with_no_currently_valid_snapshot(tmp_path):
     ...
     anchors = scout.extract_anchors(
@@ -732,6 +903,107 @@ def scout_client(tmp_path):
...
@@ -976,3 +1248,329 @@ def test_runs_route_lists_a_completed_job(scout_client):
     assert len(runs) == 1
...
+
... [diff_bound] apps/backend/tests/test_scout.py: 257 more diff lines omitted — Read the file for full detail
...
diff --git a/apps/backend/tests/test_micro_no_referee_evidence_guard.py b/apps/backend/tests/test_micro_no_referee_evidence_guard.py
new file mode 100644
index 0000000..a3e45ac
--- /dev/null
+++ b/apps/backend/tests/test_micro_no_referee_evidence_guard.py
@@ -0,0 +1,208 @@
