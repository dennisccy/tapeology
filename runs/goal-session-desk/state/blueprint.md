...
| J-19 Top-up library-reach disclosure (post-fetch newest-frozen-bar date per pair, additive to the already-registered "Top-up run records" row) -- record half shipped iter-32; day-precision display fix RESOLVED at iter-34 (iter-33's spec-ordered fix was never dispatched a developer; see "RESOLVED at iter-34" note below) | `/desk` (existing Top-up Runs section; zero new section/control, zero new ranked-table column) | Desk |
| J-20 Screen comparison -- discloses how the currently-displayed screen differs from the screen recorded immediately before it (rank/side/band_class/distance_bps/basis_as_of diff, entered/left sets) -- IN BUILD at iter-35 | `/desk` (new read-only "Screen Comparison" section, rendered after the ranked briefing table -- no standalone page) | Desk |
| J-21 Screen-pin disclosure -- states, before the click, the five pins a screen run would resolve now and whether a snapshot is already recorded under them (match / differ / empty) -- IN BUILD at iter-36 | `/desk` (existing Provenance panel + one new line beside the existing Run Screen control -- no new section, no standalone page) | Desk |
...
| **Screen comparison (rank/side/band_class/distance_bps/basis_as_of diff between two recorded snapshots)** — NEW at iter-35 (J-20) | new `app/research/desk_screen_diff.py` (name at build discretion) | `GET /research/desk/screen/compare` (exact path/params at build discretion, e.g. `?id=<compare id>&base=<base id>`) | shape: `{"compare": {"id": str, "screen_date": str, "as_of": str, "created_utc": str, "bar_store_signature": str, "universe_snapshot_id": str\|null, "ranked_count": int>=0, "skipped_count": int>=0}, "base": <same shape>\|null, "base_resolution": "explicit"\|"default_prior_date"\|"none_earlier", "rows": [{"symbol": str, "status": "compared"\|"entered"\|"left", "compare_rank": int\|null, "base_rank": int\|null, "rank_change": int\|null, "compare_side": "support"\|"resistance"\|null, "base_side": "support"\|"resistance"\|null, "compare_band_class": "A"\|"B"\|"C"\|null, "base_band_class": "A"\|"B"\|"C"\|null, "compare_distance_bps": float\|null, "base_distance_bps": float\|null, "compare_basis_as_of": str\|null, "base_basis_as_of": str\|null, "skip_reason": "no_bars"\|"no_basis"\|null}], "identical": bool, "counts": {"compared": int>=0, "rank_changed": int>=0, "side_changed": int>=0, "entered": int>=0, "left": int>=0}}` — a NEW value with exactly one owner and one serving endpoint, persisting NOTHING (no store, no file, no cache, no index, no new `Config` field, no new MCP tool -- `get_endpoint`'s existing `/research/` allowlist already reaches the new path with zero code change; J-06's exactly-17-tool contract is unaffected). Every per-symbol field is copied VERBATIM from the two snapshots' own recorded rows via `ScreenStore.list()` -- zero `compute_tradability` call, zero `BarStore`/`bar_index`/dataset read, zero re-rank, zero re-score; the default base is the record `?date=` already serves for the greatest strictly-earlier `screen_date`, an explicit `base=<id>` overrides it, and the same two ids always reproduce a byte-identical body (T-6, no wall-clock field). `desk_screen.ScreenStore` stays the ONLY owner of screen snapshots/rows/skip rows and `GET /research/desk/screen` their ONLY serving endpoint -- this row records the COMPARISON only, never a second read path. Surfaces only inside a new read-only "Screen Comparison" section on `/desk`, rendered AFTER the ranked briefing table -- no new ranked-table column, no new control, no page-load compute trigger. See "IN BUILD at iter-35" below for the full build-time scope note. |
| **Screen-pin resolution (the five pins a run would resolve now, and whether a snapshot is already recorded under them)** — NEW at iter-36 (J-21) | new `app/research/desk_screen_pins.py` (name at build discretion) | `GET /research/desk/screen/pins` (exact path at build discretion; `screen_date` REQUIRED query param -- 422 if absent, mirrors `POST /research/desk/screen/compute`'s own required-param convention) | shape: `{"screen_date": str, "as_of": str, "universe_snapshot_id": str \| null, "config_fingerprint": str, "bar_store_signature": str \| null, "members_total": int>=0, "recorded": {"id": str, "screen_date": str, "created_utc": str, "bar_store_signature": str, "ranked_count": int>=0, "skipped_count": int>=0} \| null}` -- honest empty (`universe_snapshot_id: null`, `bar_store_signature: null`, `members_total: 0`, `recorded: null`) at HTTP 200 before any universe snapshot exists, never a 404 (the `desk_universe`/`desk_screen` convention). Every pin is resolved through the accessor that ALREADY owns it -- `desk_screen.screen_as_of` for `as_of`, `UniverseStore.list()`'s own latest record id, `Config.config_fingerprint()`, and `desk_screen.compute_bar_store_signature` over `desk_coverage.get_desk_coverage`'s index-only read (T-4: zero `BarStore` reads) -- the IDENTICAL resolution `run_screen_and_record` (`desk_screen_compute.py:155-161`) already performs before its own walk, so this endpoint and a trigger's own pre-check structurally cannot disagree. `members_total` is the pinned universe record's own member count, read the way `DeskScreenComputeManager.trigger` already reads it (`len(records[-1]["members"])`, `desk_screen_compute.py:336`). The recorded-or-not answer comes from `ScreenStore.find_by_key` on exactly those five pins (`desk_screen.py:602` -- the SAME lookup J-18's pre-check already makes): when found, `recorded`'s fields are copied VERBATIM from that record's own meta; when not found, `recorded: null`. Persists NOTHING (no store, no file, no cache, no index, no new `Config` field, no new MCP tool -- `get_endpoint`'s existing `/research/` allowlist already reaches the new path with zero code change; J-06's exactly-17-tool contract is unaffected). `desk_screen.ScreenStore` stays the ONLY owner of screen snapshots/rows/skip rows and `GET /research/desk/screen` their ONLY serving endpoint -- this row records the RESOLUTION only, and computes/writes/recomputes nothing (zero `compute_tradability` calls, zero `BarStore` reads, asserted by call-count tests). Surfaces on the ALREADY-REGISTERED `/desk` canonical home in two places: the Provenance panel (`DeskProvenance`, `apps/frontend/app/desk/page.tsx:1702`, queried for the DISPLAYED snapshot's own `screen_date` -- a non-null `recorded` names that same snapshot by the store's own uniqueness invariant, so the page renders presence/absence only and derives no equality itself -- the J-20 rule) and one new descriptive line beside the Run Screen control (queried for `todayUtcDate()` -- the SAME value the trigger already submits, `apps/frontend/app/desk/page.tsx:228`/`:2350`, T-6) naming the snapshot a run today would reuse or stating that none is recorded and a run would walk `members_total` members. No new ranked-table column, no change to the ranked table itself -- J-16's measured width contract stands untouched. See "IN BUILD at iter-36" below for the full build-time scope note. |
...
NOTED at iter-33 (build-time scope, registered here BEFORE the build, per the iter-30 lesson above; no new Data-Contract row, no nav-skeleton change): the second-key confirm (`runs/goal-session-desk/iter-32/eval-confirm.md`) REJECTed iter-32's first-key GOAL_ACHIEVED on one reproduced finding -- `topupLibraryReach` (`apps/frontend/app/desk/page.tsx:878-904`) grouped the newest-reach extreme and the earlier-pairs partition by comparing `store_frozen_through_after` as a FULL microsecond-precision timestamp (`_iso_bar_epoch`, `desk_topup_compute.py:177-185`), while the render printed only the calendar day (`.slice(0, 10)`), so 202 of 303 pairs shown under "Pairs recorded earlier" printed the SAME day the reach line named as newest -- plus J-19 acceptance step 4's own "a short list" wording, rendered as all 303 pairs (iter-32 dev handoff called it short but did not cap it), and the `[NEW]`-flagged demo-narrator walkthrough, never recorded because iter-32 dispatched at `lean` despite its own spec asking `full`. iter-33 fixes `topupLibraryReach` to group/compare at calendar-day granularity (the SAME granularity already rendered), caps the rendered earlier-pairs list to at most 20 rows with an honest shown/true-total disclosure only when the true total exceeds 20, extends `test_desk_topup_library_reach_guard.py` with the day-truncation and cap assertions (plus seeded-violation counterparts), refreshes `journey-scripts/J-17.json` and `journey-scripts/J-19.json` to stable substrings (the `J-18.json` post-iter-30 hardening precedent) instead of today's exact pinned counts/dates, and records J-19's first `[NEW]`-flagged demo-narrator walkthrough against the ambient `:3301`/`:8301` pair (iter-31 lesson: no scoped rig). Zero diff to `_pair_window`, `run_topup`, `desk_topup_log.py`, or the stored shape/precision of `store_frozen_through`/`store_frozen_through_after` -- those values are already correct; only the frontend's display-time grouping of the already-served value is corrected, so this is a correctness/documentation fix to the ALREADY-REGISTERED "Top-up run records" row's iter-32 addition, not a new Data-Contract row, new endpoint, new page, or nav-skeleton change. Depth is `full` this iteration (Full trigger 1: structural/cross-cutting -- the fix's own correctness spans the frontend derivation function, its structural guard test, two OTHER journeys' golden replay scripts, and a first-ever demo-narrator walkthrough capture, a blast radius no single journey's own test coverage reaches; separately, the engine's own binding recommendation for this iteration is already `full` per `session.json`, set after the confirm's REJECT).

RESOLVED at iter-34 (build-time scope, registered here to land in the SAME commit as the code, per the iter-30 lesson above; no new Data-Contract row, no nav-skeleton change): iter-33's own spec ordered exactly the fix registered in its NOTED entry above (day-precision grouping in `topupLibraryReach`, a 20-row cap with honest shown/true-total disclosure, guard-test extensions, and the two golden-script refreshes) but the engine dispatched iter-33 at `Depth: evidence` -- first a budget-breach demotion, then SPEED-9's evidence backstop firing because J-19 was still recorded `passing` at trigger time -- so no developer ran and the product diff for that iteration was empty (confirmed directly at iter-34's start: `apps/frontend/app/desk/page.tsx`'s `topupLibraryReach` still compared full microsecond-precision `store_frozen_through_after` strings for grouping while rendering only the day, still had no cap on the `earlier` array, and `test_desk_topup_library_reach_guard.py` still had no day-truncation or cap assertion; `journey-scripts/J-19.json` step 4 still asserted `"AAPL 4h — 2026-07-30"` as an earlier row, i.e. it still enshrined the bug). The prior verdict (iter-33: `ESCALATE`) made iter-34's depth `full` mandatorily, which finally put a developer back in the loop. iter-34 lands exactly the same, previously-registered fix, in this commit: `topupLibraryReach` now derives one day-truncated key per outcome (`store_frozen_through_after.slice(0, 10)`) and groups/compares at that calendar-day precision (never the raw microsecond field) so a pair sharing the newest day's own calendar date can never be misclassified as "earlier"; the rendered "Pairs recorded earlier" list is capped at `EARLIER_PAIRS_DISPLAY_CAP` (20) rows via a new `earlierTotal` field that preserves the true count separately, with a `showing <shown> of <true total>` disclosure rendered only when the true total exceeds 20; `test_desk_topup_library_reach_guard.py` gains day-truncation and cap assertions (plus render-wiring assertions for the cap disclosure's placement and gating), each with a seeded-violation counterpart; and `journey-scripts/J-19.json` is repointed to stable substrings/testid-existence checks and no longer asserts the bug's own contradictory row as correct. Re-grouping the SAME ambient run (`topup-2026-07-31-8fb5c9a1f737`, 404 pairs) at calendar-day precision flips the old buggy 101/303 split to its honest inverse: 303 pairs share the newest day (2026-07-30) and 101 pairs are genuinely earlier (all recorded 2026-07-27) -- true total 101 exceeds the 20-row cap, so the new "showing 20 of 101" disclosure is live-observable on the ambient page today, not merely a synthetic/unit-level case. J-19's first `[NEW]`-flagged demo-narrator walkthrough is recorded by the demo-narrator step immediately following this commit, against the ambient `:3301`/`:8301` pair, narrated from the actually-rendered post-fix page (the iter-33 lesson: a walkthrough recorded before code lands can only narrate the old state). Zero diff to `_pair_window`, `run_topup`, `desk_topup_log.py`, `bars.py`, `bar_index.py`, `desk_coverage.py`, `desk_screen.py`, `tradability.py`, `levels.py`, or the stored shape/precision of `store_frozen_through`/`store_frozen_through_after` -- those are already correct; only the frontend's display-time grouping of the already-served value is corrected, so this is a correctness fix to the ALREADY-REGISTERED "Top-up run records" row's iter-32 addition, not a new Data-Contract row, new endpoint, new page, or nav-skeleton change. No real ambient top-up trigger this iteration -- the existing `topup-2026-07-31-8fb5c9a1f737` run already carries genuine cross-timeframe reach-date variance sufficient to exercise the fix live, and `lessons.md` iter-32 already documents the cost of triggering a further real run against a "latest wins" panel (it invalidates sibling golden scripts). `journey-scripts/J-17.json` was already refreshed at iter-33 (now on HEAD, commit `efef1c1`) -- not touched again this iteration. Depth is `full` this iteration (Full trigger 3: the prior evaluator verdict was `ESCALATE`, which makes `full` mandatory with no exceptions per the depth-binding rule; it is also the engine's own binding recommendation for this iteration per `session.json`).
IN BUILD at iter-35 (build-time scope for J-20, registered here BEFORE the build, per the iter-30 lesson above; NEW Data-Contract row registered above, no nav-skeleton change -- the new section lives on the ALREADY-REGISTERED `/desk` canonical home): the goal-proposer's thirteenth post-GOAL_ACHIEVED journey (era closed GOAL_ACHIEVED at iter-34, awaiting the second-key confirm when the proposer promoted this cycle's top-scored proposal; proposer rationale: `state/proposer-result.json` + `state/enhancement-proposals.jsonl`, measured 2026-07-31 read-only over the 12 recorded screen snapshots: pairing each with the record for its own greatest strictly-earlier `screen_date`, four consecutive pairs (07-28->07-29->07-30->07-31) change NOTHING in any ranked row but `basis_age_days`, while a further seven pairs churn heavily (up to 95 of 100 rows moving rank, 12 flipping side, PLTR 7->84) -- and `/desk` renders both extremes identically, because no desk module and no line of `apps/frontend/app/desk/page.tsx` ever compares two snapshots). Unlike J-09/J-10/J-18 (each a wholly new DURABLE store) and like J-08/J-11/J-13/J-14/J-15/J-17/J-19 (each additive to an already-registered row) -- J-20 sits in a THIRD shape: it adds ONE wholly new Data-Contract row (the comparison, registered above) that is nonetheless a PURE, stateless read over two ALREADY-persisted, immutable snapshots -- zero new store, zero new file, zero new index, zero new `Config` field. No new page (the new "Screen Comparison" section lives on the ALREADY-REGISTERED `/desk` canonical home, same as every prior desk journey), no nav-skeleton change, no new endpoint outside the one new GET registered above, no new MCP tool (`get_endpoint`'s existing `/research/` allowlist already reaches the new path with zero code change -- J-06's exactly-17-tool contract is unaffected). Zero diff to `desk_screen.py`'s recorded row/snapshot shapes, five-pin key, or rank key, and zero diff to `tradability.py`/`levels.py`/`bars.py`/`bar_index.py`/`desk_coverage.py`/`StructureChart.tsx` -- the comparison reads `ScreenStore.list()` verbatim and computes nothing a canonical owner has not already computed. No ranked-table column and no change to the ranked table itself -- J-16's measured width contract and every one of the 19 stored golden replay scripts stand untouched (the new section renders AFTER the ranked table specifically so the replay tool's first-match text search cannot resolve into it -- goal.md step 6). Depth is `full` this iteration (Full trigger 1: structural/cross-cutting -- a brand-new, never-before-built full-stack journey whose interaction spans a new backend module, a new endpoint, and a new `/desk` section rendered beside nine already-shipped sections' testids/tooltip/copy contracts, with no existing single journey's own test coverage spanning that blast radius -- also because J-20's acceptance names a first-ever `[NEW]`-flagged demo-narrator walkthrough for this specific disclosure, the iter-12/13/26/29/32 lesson (`lessons.md`) that a `lean`-dispatched iteration cannot score a brand-new walkthrough clause within its own run, and the binding depth recommendation's own "brand-new full-stack journey" escape condition, since the evaluator's `lean` recommendation for this iteration predates the goal-proposer's promotion of J-20 this cycle -- the same citation pattern iterations 15, 17, 23, 24, 26, 29 and 32 used for their own brand-new journeys). No sibling proposal from this cycle was promoted alongside it (`n_proposals: 2`, one promoted; the backlogged sibling -- the displayed screen's own `bar_store_signature` vs the store's CURRENT signature -- is deferred, per `proposer-result.json`'s own summary, to keep this iteration's scope to the one promoted journey). No real screen run is triggered this iteration -- the ambient ledger's own 12 recorded snapshots already carry both the identical-state and churned-state pairs the acceptance names, so all required evidence is captured over already-persisted, immutable records.

IN BUILD at iter-36 (build-time scope for J-21, registered here BEFORE the build, per the iter-30 lesson above; NEW Data-Contract row registered above, no nav-skeleton change -- the new disclosure lives on the ALREADY-REGISTERED `/desk` canonical home): the goal-proposer's fourteenth post-GOAL_ACHIEVED journey (era closed GOAL_ACHIEVED at iter-35, awaiting the second-key confirm when the proposer promoted this cycle's top-scored proposal; proposer rationale: `state/proposer-result.json` + `state/enhancement-proposals.jsonl`, measured 2026-07-31 read-only over the frozen artifacts, no service started, no product code run: the index currently resolves `bar_store_signature 2ce14e8f252966f7`, a value NONE of the 12 recorded screen snapshots carries (the 06:52->06:56Z top-up moved `MAX(window_end_utc)` for 404 of 404 pinned member x timeframe pairs), so the SAME Run Screen click has recorded both a 1m41s 101-member walk and, elsewhere, a 14ms/16ms `reused` short-circuit with nothing on the page distinguishing which one a fresh click will get; two sibling vision-gap proposals -- the coverage view (`GET /research/desk/coverage` still has zero frontend clients) and the universe surface (`GET`/`POST /research/desk/universe` still has zero frontend clients) -- were both measured and NOT promoted, scored lower than J-21 and deferred). Like J-08/J-11/J-13/J-14/J-15/J-17/J-19 (each additive to an already-registered row) and like J-20 (a pure stateless read over already-persisted records), J-21 adds ONE wholly new Data-Contract row (the pin resolution, registered above) that PERSISTS NOTHING -- zero new store, zero new file, zero new index, zero new `Config` field. No new page (the disclosure lives on the ALREADY-REGISTERED `/desk` canonical home, inside the ALREADY-REGISTERED Provenance panel plus one new line beside the ALREADY-REGISTERED Run Screen control), no nav-skeleton change, no new endpoint outside the one new GET registered above, no new MCP tool (`get_endpoint`'s existing `/research/` allowlist already reaches the new path with zero code change -- J-06's exactly-17-tool contract is unaffected). Zero diff to `desk_screen.py`/`desk_screen_compute.py`/`desk_coverage.py`/`tradability.py`/`levels.py`/`bars.py`/`bar_index.py`/`StructureChart.tsx` -- every pin is resolved through the SAME accessor that already owns it (`screen_as_of`, `UniverseStore.list()`, `Config.config_fingerprint()`, `compute_bar_store_signature` over `desk_coverage`), the identical resolution `run_screen_and_record` already performs before its own walk, so this endpoint and a trigger's own pre-check structurally cannot disagree; the recorded-or-not answer is `ScreenStore.find_by_key` on those five pins, the SAME lookup J-18's pre-check already makes. No new ranked-table column and no change to the ranked table -- J-16's measured width contract and every one of the 20 stored golden replay scripts stand untouched (goal.md's own J-20-precedent static-sweep clause extends to this journey's own added strings). Depth is `full` this iteration (Full trigger 1: structural/cross-cutting -- a brand-new, never-before-built full-stack journey whose interaction spans a new backend module, a new endpoint, and edits to two already-shipped `/desk` render surfaces (the Provenance panel, the Run Screen control's sibling line) with no existing single journey's own test coverage spanning that blast radius -- also because J-21's acceptance names a first-ever `[NEW]`-flagged demo-narrator walkthrough for this specific disclosure, the iter-12/13/26/29/32/35 lesson (`lessons.md`) that a `lean`- or `evidence`-dispatched iteration cannot score a brand-new walkthrough clause within its own run, and the binding depth recommendation's own "brand-new full-stack journey" escape condition, since the evaluator's `evidence` recommendation for this iteration predates the goal-proposer's promotion of J-21 this cycle -- the same citation pattern iterations 15, 17, 23, 24, 26, 29, 32 and 35 used for their own brand-new journeys). Because the ambient store today naturally shows ONLY the differ state (no recorded screen carries the currently-resolved pins), the match and empty states are captured on a FIXTURE-SCOPED rig rather than by a real ambient Run Screen click, which would create a fresh 'latest' snapshot and invalidate sibling golden scripts pinned to the current one (the `lessons.md` iter-32 rule) -- the differ state alone is captured on the ambient `:3301`/`:8301` pair, read-only, zero risk. No sibling proposal from this cycle was promoted alongside it (`n_proposals: 3`, one promoted; the two backlogged siblings are the coverage-view and universe-surface vision gaps above, deferred to keep this iteration's scope to the one promoted journey).
