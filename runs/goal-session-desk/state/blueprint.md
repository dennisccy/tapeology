...
└── Desk         /desk         [shipped iter-4, J-04] universe snapshot + coverage summary, screen
                                ...
                                below. iter-15 (J-11, IN BUILD) adds a descriptive `history`
                                column (completed daily-session count + start date) to the
                                ranked table, plus a two-line addition to the row's existing
                                composite hover tooltip — no new section, no new control.
```
