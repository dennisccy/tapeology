...
Files changed: 31. Shown in full: 31.
...
@@ -493,3 +505,26 @@ def cancel_tick_recorder_compute(
...
+    every registered universe (never the raw secret, only its commitment), and both ledgers' own
+    chain-verification verdicts. Never 404/500 on an empty vault -- the desk router's established
+    never-404-on-absence convention: an honest empty ``shards``/``universes`` before any universe
+    is ever registered (registration is a step-4, operator-attended act, out of THIS iteration's
+    scope)."""
...
## Excluded-path stat (dependency/lockfile visibility)
 ...
 .../state/assumptions.md                           | 59 ++++++++++++++++++++++
 .../state/golden-gaps                              |  1 -
 runs/goal-session-rapid-microscope/telemetry.jsonl | 59 ++++++++++++++++++++++
 .../trace/trace.jsonl                              | 13 +++++
