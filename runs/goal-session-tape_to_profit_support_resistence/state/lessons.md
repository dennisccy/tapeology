...
## iter-6 — 2026-07-06T18:15:00Z

**Verdict:** GOAL_ACHIEVED
**Lesson:** The reused-verbatim `overfit = train_positive and not survivor` formula labels an *abstaining* candidate as `overfit=true`: on the committed PG fixtures `structure_tape` trades 0 times on train, but the train delta reads positive purely because champion `v1` lost money on that window — so an untraded candidate is tagged "overfit." It is non-gating (promotion hinges only on `survivor`, correctly False) and the honest datum `candidate_n=0` is in the per-dataset breakdown, so it is not a defect — but the label is semantically loose. Any future era that generalizes the sweep further (or surfaces `overfit` in a UI/report headline) should distinguish "abstained (n=0)" from "genuinely overfit a spurious train edge" rather than trust the boolean at face value.
**Applies to:** any future iter that generalizes `pnl_scan.run_sweep` / the strategy-comparison report, or renders the `overfit` flag on a surface a human reads.
