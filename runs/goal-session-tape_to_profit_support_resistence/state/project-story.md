...
Now the second capability has landed: Tapeology can look at that saved history and work out where the real support and resistance levels are — prices where the market has previously turned, or a prior day, week, or month's high, low, or close — each one scored for strength from one documented settings file, never a guess. The team directly verified that asking about a moment in time never lets a later price bar sneak into the answer, and that the same question always gives the identical answer down to the byte. Next: grouping these levels into graded confluence zones a future strategy can react to.

## What it can do today

The product lets users type in a stock ticker and watch live trade-by-trade action to see who's in control, journal trading ideas, run replay studies against past data, backtest a strategy into an honest profit-or-loss report beside a random-guessing comparison, view that scorecard on a Performance page, and let AI assistants read all of it directly. The last two chapters' price-history and price-level work are not yet visible in the app — they are groundwork for a future visible feature.
