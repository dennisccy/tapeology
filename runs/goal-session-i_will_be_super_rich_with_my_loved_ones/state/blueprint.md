...
| 24 | **Taxonomies + research display copy** (setups, flags, tags, verdict/stance enums) | Backend taxonomy module | `GET /research/taxonomy` | frontend hardcodes none of them. **Iter-13 (additive):** the **mistake-tag catalog** ships (`chased`, `entered_before_confirmation`, `ignored_rejection`, `ignored_risk_flags`, `moved_invalidation` *(self-assessed)*, `no_clear_setup`, `wrong_setup_type`, `overstayed`, `other` + required note) with display copy, served by this same endpoint; the review SAVE flow (`POST …/review`) lands with J-57. **Iter-14 (additive):** the review SAVE flow ships as row 28; grade/final-statement-status display copy is served additively by this same endpoint (frontend hardcodes none). **Iter-15 (additive):** excursion display copy ships via this same endpoint — ternary-outcome labels (`+1R_first \| −1R_first \| neither_within_horizon`), the `truncated` label, not-tracked / not-applicable copy, and the two population titles; the frontend hardcodes none |
