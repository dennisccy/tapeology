...
| 19 | **Execution checks + outcome × process grades** | Computed once at resolution (`POST /research/thesis/{id}/resolve`); persisted | `GET /research/journal/{id}` | enum labels from evidence-backed checks; never numeric scores |
