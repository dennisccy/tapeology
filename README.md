...
- **Honest connection-state status light** — the status indicator now has two additional states beyond connecting/live/stale/paused/closed: "waiting" (amber pulse — connected but no data yet) and "failed" (rose — the feed broke after connecting). The dot never shows a confident green "live" over an empty or broken tape.
