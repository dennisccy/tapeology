...
- **Hints log in the Journal** — a third "Hints" view in the Journal page shows every hint the cockpit has ever fired, in order, with its pattern, plain-language evidence, data feed, config fingerprint, and wall-clock timestamp. The log is permanent and append-only; entries are never edited or deleted.
