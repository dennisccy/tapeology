...
- **Journal page** — a dedicated Journal page (reachable from the top navigation bar) shows every thesis you have ever declared — active, resolved, expired, and abandoned — in a filterable table. Each row shows the declared date, ticker, source, data feed, setup, direction, and current status. You can filter by status; the table is honest about system-owned outcomes versus user-driven resolutions.
