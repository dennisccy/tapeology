...
- **Fast Historical loading** — trades and quotes are fetched concurrently; re-watching the same symbol and window is near-instant (the result is cached for the session). The cockpit shows a warm, meaningful read quickly after the fetch completes.
- **Actionable "window too large" error** — when a Historical window is too high-volume to load within budget, the error panel shows "that window is very high-volume — try a shorter range" instead of a vague retry prompt.
