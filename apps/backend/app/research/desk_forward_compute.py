...
**No run-log store in v1.** The screen compute only gained its durable run log at J-18; the
forward surface starts minimal -- the manager snapshot covers in-flight legibility and the
append-only ``ForwardStore`` itself is the durable outcome (every completed compute is a record;
an interrupted one honestly leaves nothing)."""
